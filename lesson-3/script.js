...
class GoodsItem {
    ...
    getHtml() {
        ...
        return `<div class="goods-item">${img}<h3>${this.title}</h3><p>${this.price}</p><button class="addClick" onclick="addBasket(${this.id})">Добавить</button></div>`;
    }
}
...
class GoodsList {
    ...
    onFetchSuccess(data) {
        this.goods = data.map(({ id, title, price }) => new GoodsItem(id, title, price));
        this.render();
    }
...
class BasketItem {
    ...
    render() {
        return `<div class="basket-item"><div class="img-basket"></div><div class="basket-info"><h3>${this.title}</h3><p>${this.price}</p></div><button class='deleteItem' onclick='deleteItem(${this.id})'>&times;</button></div>`;
    }
}
// Класс корзины
class Basket {
    ...
    addToBasket(id) {
        let toBasket;
        goodsList.goods.forEach(function (item) {
            if (id == item.id) {
                toBasket = {
                    id: item.id,
                    title: item.title,
